-- NSE Intelligence Platform Database Schema
-- Created for OSINT-driven Nairobi Stock Exchange monitoring
...
CREATE TABLE messages (
    ...
    article_id UUID REFERENCES news_articles(id), -- Link to source if applicable
...
CREATE INDEX idx_company_mentions_article_id ON company_mentions(article_id);
CREATE INDEX idx_company_mentions_company_id ON company_mentions(company_id);
CREATE INDEX idx_sentiment_analysis_company_id ON sentiment_analysis(company_id);
CREATE INDEX idx_sentiment_analysis_article_id ON sentiment_analysis(article_id);
CREATE INDEX idx_detected_events_company_id ON detected_events(company_id);
