...
export class StockPricesService {
  ...
  async getTickerBySymbol(ticker: string) {
  ...
  async getTickerById(companyId: string) {
    const stock = this.tickers.find(t => t.companyId === companyId);
    if (!stock) {
      throw new NotFoundException(`Stock with company ID ${companyId} not found`);
    }
  ...
  async getPriceHistory(ticker: string, range = '1m', interval = '1d') {
    const stock = this.tickers.find(t => t.ticker.toUpperCase() === ticker.toUpperCase());
    if (!stock) {
      throw new NotFoundException(`Stock ${ticker} not found`);
    }
  ...
  async getSectorPerformance() {
    const sectorMap: Record<string, { stocks: any[]; totalMarketCap: number }> = {};
  ...
  private isTradingHours(): boolean {
    ...
    // NSE trading hours: Mon-Fri, 9:30 AM - 3:00 PM EAT
    if (day === 0 || day === 6) return false;
    if (eatHour < 9 || (eatHour === 9 && now.getUTCMinutes() < 30)) return false;
    if (eatHour >= 15) return false;
    return true;
  }
}
