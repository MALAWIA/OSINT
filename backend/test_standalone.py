#!/usr/bin/env python
"""
Standalone Test Suite for NSE Authentication System
Tests core functionality without Django setup
"""
import os
import sys
import json
...
class StandaloneAuthenticationTest:
    """Standalone test suite for authentication functionality"""
    ...
    def test_unique_code_generation(self):
        ...
        try:
            ...
            self.assertEqual(len(unique_codes), 10, "Codes are not unique")
            
            # Test format validation
            for code in codes:
                parts = code.split('-')
                self.assertEqual(len(parts), 3, f"Invalid format for {code}")
                self.assertEqual(parts[0], 'NSE', f"Invalid prefix for {code}")
                self.assertEqual(len(parts[1]), 12, f"Invalid middle part for {code}")
                self.assertEqual(len(parts[2]), 4, f"Invalid suffix for {code}")
                self.assertTrue(parts[1].isalnum(), f"Middle part not alphanumeric for {code}")
                self.assertTrue(parts[2].isdigit(), f"Suffix not numeric for {code}")
        ...
        try:
            ...
            def verify_password_with_salt(password, hashed_password, salt):
                try:
                    ...
                    salt_part = decoded_hash[:32]
                    password_salt = (password + salt_part).encode('utf-8')
                    hash_obj = hashlib.sha256(password_salt)
            ...
            self.assertTrue(is_valid, "Password verification failed")
            
            # Test wrong password
            is_invalid = verify_password_with_salt("WrongPassword", hashed_password, salt)
            self.assertFalse(is_invalid, "Wrong password verification passed")
    ...
    def test_authentication_logic(self):
        ...
        try:
            ...
            self.assertIsNotNone(auth_user, "Successful authentication failed")
            ...
            self.assertIsNone(failed_auth, "Wrong unique code authentication passed")
            
            # Test wrong password
            failed_auth = authenticate_user('user1@example.com', 'WrongPassword', 'NSE-A7B3K9M2C4D8-1234')
            self.assertIsNone(failed_auth, "Wrong password authentication passed")
            
            # Test non-existent user
            failed_auth = authenticate_user('nonexistent@example.com', 'TestPassword123!', 'NSE-TEST-CODE-5678')
            self.assertIsNone(failed_auth, "Non-existent user authentication passed")
    ...
    def test_security_validation(self):
        ...
        try:
            ...
            self.assertTrue(result['is_valid_format'], "Valid code format validation failed")
            self.assertTrue(result['is_secure'], "Valid code security validation failed")
            self.assertGreater(result['entropy_score'], 0.5, "Valid code entropy score too low")
            
            # Test weak code
            weak_code = "NSE-AAAAAAAAAAAA-1111"
            result = validate_code_security(weak_code)
            
            self.assertTrue(result['is_valid_format'], "Weak code format validation failed")
            self.assertFalse(result['is_secure'], "Weak code security validation passed")
            self.assertLessThan(result['entropy_score'], 0.3, "Weak code entropy score too high")
    ...
    def test_api_simulation(self):
        ...
        try:
            ...
            self.assertEqual(response['status'], 201, "Registration simulation failed")
            ...
            self.assertEqual(response['status'], 200, "Login simulation failed")
            ...
            self.assertEqual(response['status'], 401, "Invalid login simulation failed")
    ...
    def test_performance(self):
        ...
        try:
            ...
            self.assertEqual(len(codes), 1000, "Code generation count mismatch")
            self.assertLessThan(generation_time, 5.0, "Code generation too slow")
            
            # Test uniqueness
            unique_codes = set(codes)
            self.assertEqual(len(unique_codes), 1000, "Codes not unique")
    ...
    def generate_test_report(self):
        ...
        print(f"🔐 Test Type: Standalone (No Django Setup)")
        print("="*80)
...
def run_standalone_tests():
    ...
    print("🧪 RUNNING STANDALONE AUTHENTICATION TESTS")
    print("="*80)
    
    test_suite = StandaloneAuthenticationTest()
