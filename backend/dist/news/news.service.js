...
let NewsService = class NewsService {
    ...
    async findByCompany(companyId, limit = 20) {
        return this.newsArticles
            .filter(article => article.companies.includes(companyId) && article.isActive)
            .sort((a, b) => new Date(b.publishedAt).getTime() - new Date(a.publishedAt).getTime())
            .slice(0, limit);
    }
    async findByKeyword(keyword, limit = 20) {
        const lowerKeyword = keyword.toLowerCase();
        return this.newsArticles
            .filter(article => article.isActive && (article.title.toLowerCase().includes(lowerKeyword) ||
            article.summary.toLowerCase().includes(lowerKeyword) ||
            article.keywords.some((k) => k.toLowerCase().includes(lowerKeyword))))
            .sort((a, b) => new Date(b.publishedAt).getTime() - new Date(a.publishedAt).getTime())
            .slice(0, limit);
    }
    async findBySentiment(sentiment, limit = 20) {
        return this.newsArticles
    ...
    async search(query, limit = 20) {
        ...
        return this.newsArticles
            .filter(article => article.isActive && (article.title.toLowerCase().includes(lowerQuery) ||
